...
def clean_data(df):
    ...
    if "Gender" in df.columns:
        ...
        # Replace invalid gender values with NaN
    ...
    if "Phone Number" in df.columns:
        ...
        df.loc[
            ~df["Phone Number"].str.len().between(7, 15),
            "Phone Number"
        ] = pd.NA
    ...
    if "Cholesterol" in df.columns:
        ...
        df.loc[
            (df["Cholesterol"] < 50) | (df["Cholesterol"] > 400),
            "Cholesterol"
        ] = pd.NA
...
def summary_statistics(df):
    ...
    for col in numeric_cols:
        ...
        print(f"Mean: {df[col].mean()}")         # Average
        print(f"Median: {df[col].median()}")     # Middle value
        print(f"Min: {df[col].min()}, Max: {df[col].max()}")  # Range
        print("Quartiles:")
        print(df[col].quantile([0.25, 0.5, 0.75]))  # Quartiles
